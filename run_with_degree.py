...
def compute_degree_encoding(edge_index, num_nodes):
    """
    计算度编码
    :param edge_index: PyTorch Geometric 格式的边索引 (2, num_edges)
    :param num_nodes: 图中节点的数量
    :return: 度编码 (num_nodes, 1)
    """
    # 计算每个节点的度
    deg = degree(edge_index[0], num_nodes, dtype=torch.float)  # (num_nodes,)
    return deg.unsqueeze(1)  # 转换为 (num_nodes, 1)
...
class GraphTransformer(nn.Module):
    def __init__(self, num_layers, in_features, hidden_features, out_features, num_heads):
        ...
        self.layers = nn.ModuleList([
            GraphTransformerLayer(in_features + 1 if i == 0 else hidden_features, hidden_features, num_heads)
...
def run():
    ...
    pe = compute_degree_encoding(data.edge_index, data.x.size(0))
    print(pe)
